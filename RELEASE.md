...
- Replaced the unmaintained `MsBuildPipeLogger` packages with `XenoAtom.MsBuildPipeLogger` (#356). Buildalyzer now runs every build out-of-process (`dotnet msbuild` / `MSBuild.exe`) and reads the resulting events over the pipe as XenoAtom's version-independent `PipeBuildEventArgs`, so the core library no longer has any `Microsoft.Build.*` dependency (no engine, no `MSBuildLocator`, no `Microsoft.Build.Framework`). Consequences for consumers:
  ...
  - `IAnalyzerResults.BuildEventArguments` is now an `ImmutableArray<XenoAtom.MsBuildPipeLogger.PipeBuildEventArgs>` instead of `ImmutableArray<Microsoft.Build.Framework.BuildEventArgs>`.
