...
- Replaced the unmaintained `MsBuildPipeLogger` packages with `XenoAtom.MsBuildPipeLogger` (#356). Its receiver deserializes events with the real MSBuild `BuildEventArgsReader`, which must be at least as new as the MSBuild that wrote them. Buildalyzer therefore no longer ships its own `Microsoft.Build` assemblies; it registers `Microsoft.Build.Locator` at startup and loads MSBuild from the installed .NET SDK so the reader always matches the SDK's MSBuild that produced the binary log. Consequences for consumers:
  - The host process must run on a .NET runtime at least as new as the SDK it builds with (the standard `MSBuildLocator` requirement).
  - `Microsoft.Build*` is no longer a transitive package dependency of Buildalyzer. Code that uses Buildalyzer's MSBuild-typed API (`AnalyzerManager.Analyze`/`IProjectAnalyzer.AddBuildLogger`/`BuildLoggers` with `Microsoft.Build.Framework.ILogger`, or `AddBinaryLogger` which exposes `Microsoft.Build.Logging.BinaryLogger`) must add its own `Microsoft.Build`/`Microsoft.Build.Framework` reference with `ExcludeAssets="runtime"`.
  - `AnalyzerManager.Analyze(binLogPath)` now reads the binary log with MSBuild's own `BinaryLogReplayEventSource` (loaded from the SDK via `MSBuildLocator`) instead of `MSBuild.StructuredLogger`, so the reader always matches the SDK's MSBuild that wrote the log. The `MSBuild.StructuredLogger` dependency has been dropped.
