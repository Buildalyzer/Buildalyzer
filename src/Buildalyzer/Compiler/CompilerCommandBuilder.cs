...
internal static class CompilerCommandBuilder
{
        ...
        IReadOnlyDictionary<string, List<CompilerInputItem>> taskInputs)
    {
        ...
        return command with
        {
            ...
            Arguments = arguments,
            SourceFiles = [.. sources.Select(i => Resolve(projectDirectory, i.Spec))],
            MetadataReferences = [.. references.Select(i => i.Spec)],
            AnalyzerReferences = [.. Items(taskInputs, "Analyzers").Select(i => Resolve(projectDirectory, i.Spec))],
            AdditionalFiles = [.. Items(taskInputs, "AdditionalFiles").Select(i => Resolve(projectDirectory, i.Spec))],
            AnalyzerConfigPaths = [.. Items(taskInputs, "AnalyzerConfigFiles").Select(i => Resolve(projectDirectory, i.Spec))],
            EmbeddedFiles = [.. embedded.Select(spec => Resolve(projectDirectory, spec))],
